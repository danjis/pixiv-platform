...
public interface ArtworkRepository extends JpaRepository<Artwork, Long>, JpaSpecificationExecutor<Artwork> {
        ...
        @Query("SELECT COALESCE(SUM(a.viewCount), 0) FROM Artwork a WHERE a.status = 'PUBLISHED'")
        long sumViewCount();

        @Query("SELECT COALESCE(SUM(a.likeCount), 0) FROM Artwork a WHERE a.status = 'PUBLISHED'")
        long sumLikeCount();

        @Query("SELECT COALESCE(SUM(a.favoriteCount), 0) FROM Artwork a WHERE a.status = 'PUBLISHED'")
        long sumFavoriteCount();

        @Query("SELECT COALESCE(SUM(a.commentCount), 0) FROM Artwork a WHERE a.status = 'PUBLISHED'")
